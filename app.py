...
app.config["GOOGLE_OAUTH_CLIENT_ID"] = "640269379621-lh564mmlg6k9ddinvpssvc24r1ha1q0b.apps.googleusercontent.com"
app.config["GOOGLE_OAUTH_CLIENT_SECRET"] = "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"
...
def home():
    if (request.method == 'GET'):
        ...
        new_history = History(email=email, name=name,
                              condition=condition, medicine=meds)
        try:
...
def explore():
    if request.method == 'POST':
        ...
        if (int(entered_date[0]) != int(current_time.year) or int(entered_date[1]) != int(current_time.month)):
            startfrom = str(current_time.year) + "-0" + \
                str(current_time.month) + "-" + "01"
        top_headlines = newsapi.get_everything(q=search_word, sort_by='relevancy',
        ...
        keyword = search_word
        # twitter tweets
        tweets = api.search(keyword, count=50, lang='en', exclude='retweets',
                            tweet_mode='extended', since=startfrom_twitter)
        tweet_sentiments = []
        demo2 = []
        demo1 = []
        pos_rev = 0
        ...
        for i in tweets:
            ...
            if (review_msg["traits"] != {}):
                ...
                tweet_sentiments.append(
                    [processed_tweet, review_msg["traits"]["wit$sentiment"][0]["value"]])
                if (review_msg["traits"]["wit$sentiment"][0]["value"] == "positive"):
                    demo2.append(1)
                    pos_rev += 1
       ...
       # demo2=[1,0,-1,0,0,-1]
        # size of x axis
        # store 0 to len(demo1)-1 in size_demo
        size_demo = [i for i in range(len(demo1))]
        pos_rev = round((pos_rev*100)/len(size_demo), 2)
        neg_rev = round((neg_rev*100)/len(size_demo), 2)
        neu_rev = round((neu_rev*100)/len(size_demo), 2)
        rev_list_precent = [pos_rev, neg_rev, neu_rev]

        return render_template('explore.html', title_list=title_list, description_list=description_list,
                               urls_headlines=urls_headlines, total_elements=total_elements, tweets=tweets, tweet_sentiments=tweet_sentiments, demo1=demo1, size_demo=size_demo, demo2=demo2, rev_list_precent=rev_list_precent)
